...
/// Force-align a WAV file against a known transcript.
/// Returns a heap-allocated JSON string:
/// [{"text":"Hello","start_ms":80.0,"end_ms":520.0}, ...]
#[no_mangle]
pub unsafe extern "C" fn qwen_asr_force_align_file(
    engine: *mut QwenAsrEngine,
    wav_path: *const c_char,
    text: *const c_char,
    language: *const c_char,
) -> *mut c_char {
    if engine.is_null() || wav_path.is_null() || text.is_null() || language.is_null() {
        return std::ptr::null_mut();
    }
    let eng = &mut *engine;
    let path = match CStr::from_ptr(wav_path).to_str() {
    ...
    let transcript = match CStr::from_ptr(text).to_str() {
        Ok(s) => s,
        Err(_) => return std::ptr::null_mut(),
    };
    let align_language = match CStr::from_ptr(language).to_str() {
        Ok(s) => s,
        Err(_) => return std::ptr::null_mut(),
    };

    let samples = match audio::load_wav(path) {
        Some(s) => s,
        None => return std::ptr::null_mut(),
    };
    let results = match align::forced_align(&mut eng.ctx, &samples, transcript, align_language) {
        Some(items) => items,
        None => return std::ptr::null_mut(),
    };
...
/// Configure whether past text is reused as streaming context.
#[no_mangle]
pub unsafe extern "C" fn qwen_asr_stream_set_past_text(engine: *mut QwenAsrEngine, enabled: i32) {
    if !engine.is_null() {
        (*engine).ctx.past_text_conditioning = enabled != 0;
    }
}

/// Greedily generate token IDs from PCM and tokenized prompt portions surrounding
/// the audio placeholder. Returns {"token_ids":[...],"finish_reason":"stop"|"length"}.
/// EOT and IM_END are excluded; all other IDs, including R2T2's pipe, are preserved.
/// The caller must serialize access to engine and free the result with free_string.
/// Invalid input, inference failure, or a caught Rust panic returns null.
#[no_mangle]
pub unsafe extern "C" fn qwen_asr_generate_pcm(
    engine: *mut QwenAsrEngine,
    samples: *const f32,
    n_samples: i32,
    before_ids: *const i32,
    n_before: i32,
    after_ids: *const i32,
    n_after: i32,
    max_tokens: i32,
    ...
    std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
        let generated = crate::generate::generate_pcm(
            &mut (*engine).ctx,
            std::slice::from_raw_parts(samples, n_samples as usize),
            std::slice::from_raw_parts(before_ids, n_before as usize),
            std::slice::from_raw_parts(after_ids, n_after as usize),
            max_tokens as usize,
        )?;
        let mut json = String::from("{\"token_ids\":[");
        for (index, token) in generated.token_ids.iter().enumerate() {
            if index > 0 {
                json.push(',');
            }
            let _ = write!(&mut json, "{}", token);
        }
        let reason = if generated.reached_limit {
            "length"
        } else {
            "stop"
        };
        let _ = write!(&mut json, "],\"finish_reason\":\"{}\"}}", reason);
